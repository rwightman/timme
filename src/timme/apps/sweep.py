...
CLI usage::
    ...
    # As a module
    python -m timme.apps.sweep sweeps/lr_sweep.yaml

    # As installed script
    timme-sweep sweeps/lr_sweep.yaml

    # Resume incomplete sweep
    timme-sweep sweeps/lr_sweep.yaml --resume

    # Custom training script
    timme-sweep sweeps/lr_sweep.yaml --train-script timme.apps.train_ssl
...
Programmatic usage::
    ...
    from timme.apps.sweep import run_sweep
...
def run_trial(
        ...
        output_dir: str,
        gpu_id: int,
        train_script: str = 'timme.apps.train_cls',
) -> SweepResult:
    ...
    # Support both module paths (timme.apps.train_cls) and script paths (train_cls.py)
...
def generate_sweep_report(
    ...
    completed = [r for r in state.results if r.get('status') == 'completed' and metric_name in r.get('metrics', {})]
    ...
    if report['best_trial']:
        ...
        _logger.info(
            f"Best trial: {report['best_trial']['trial_id']} with {metric_name}={best_value:.4f}"
        )
...
def run_sweep(
    ...
    pending_trials = [(i, t) for i, t in enumerate(all_trials) if i not in state.completed_trials]
...
def main():
    ...
    parser.add_argument(
        '--train-script',
        type=str,
        default='timme.apps.train_cls',
        help='Training script/module to use (default: timme.apps.train_cls)',
    )
