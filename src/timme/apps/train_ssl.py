#!/usr/bin/env python3
"""Self-supervised learning training script using timme.engine infrastructure.
...
CLI usage::
    ...
    # As a module
    python -m timme.apps.train_ssl --model.model vit_tiny_patch16_224 \\
        --ssl.ssl_method nepa \\
        --data.path /path/to/imagenet \\
        --scheduler.epochs 100

    # As installed script
    timme-train-ssl --model.model vit_tiny_patch16_224 --ssl.ssl_method nepa \\
        --data.path /path/to/imagenet
    ...
    # LeJEPA training
    python -m timme.apps.train_ssl --model.model vit_small_patch16_224 \\
    ...
    # With config file
    python -m timme.apps.train_ssl -c configs/nepa_vit.yaml --data.path /path/to/data

Programmatic usage (for testing)::

    from timme.apps.train_ssl import train_ssl
    from timme.engine import TrainConfig, ModelConfig, TrainDataConfig, SSLConfig, SchedulerConfig
...
from timme.task import NEPATask, LeJEPATask, load_task_ema_checkpoint
...
def parse_args() -> TrainConfig:
    ...
    try:
        ...
        raise ImportError(
            "simple-parsing is required for train_ssl. Install with: pip install simple-parsing"
        )
...
def create_ssl_task(
    ...
    """Create SSL training task from a timme encoder.

    Args:
        model: Bare timme ImageEncoder for SSL training.
    ...
    if ssl_method is None:
        raise ValueError(
            "SSL method must be specified with --ssl.ssl_method. Available methods: nepa, lejepa"
        )
    ...
    if ssl_method == 'nepa':
        ...
        if is_primary(device_env):
            _logger.info(
                f'Created LeJEPATask: proj_dim={cfg.ssl.lejepa_proj_dim}, lambda={cfg.ssl.lejepa_lamb}'
            )
    ...
    #     task = AIMTask(...)
        ...
        raise ValueError(
            f"Unknown SSL method: {ssl_method}. Available methods: nepa, lejepa"
        )
...
def train_ssl(cfg: TrainConfig) -> Optional[float]:
    ...
    Example::
        ...
        from timme.engine import TrainConfig, ModelConfig, TrainDataConfig, SSLConfig
    ...
    if cfg.ssl.ssl_method is None:
        raise ValueError(
            "SSL method must be specified in cfg.ssl.ssl_method. Available methods: nepa, lejepa"
        )
    ...
    # Create a bare timme encoder for SSL. This is the key timme path:
    # tasks train representations directly, without a classifier wrapper/head.
    ...
    if cfg.ssl.ssl_eval_metric != 'loss':
        ...
        _logger.info(
            f'Creating {cfg.ssl.ssl_eval_metric} evaluation loaders (ref: {ref_split}, probe: {probe_split})'
        )
        eval_fallback = DataSourceConfig(
            type=cfg.ssl.eval_data.type or None,
            path=cfg.ssl.eval_data.path,
        )
    ...
    # Create optimizer after task construction so projectors/decoders are included.
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            run_feature_eval = (
                cfg.ssl.ssl_eval_metric != 'loss'
                and loader_eval_ref is not None
                and loader_eval_probe is not None
                and epoch_p_1 % cfg.ssl.ssl_eval_interval == 0
            )
    ...
    if is_primary(device_env):
        _logger.info(
            ...
            '  - k-NN evaluation: python -m timme.apps.eval_knn --checkpoint <path>\n'
