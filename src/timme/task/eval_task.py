...
class EvalTask(nn.Module):
    ...
    def model(self) -> nn.Module:
        ...
        """
        module = unwrap_model(self.trainable_module)
...
class FeatureEvalModel(nn.Module):
    """Forward wrapper that makes feature extraction compile as __call__."""

    is_feature_eval_model = True
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        if hasattr(self.model, 'forward_features'):
            return self.model.forward_features(input)
...
class SSLEvalTask(EvalTask):
    ...
    def _pool_features(self, features: torch.Tensor) -> torch.Tensor:
        ...
        if self.pool == 'avg':
            traits = getattr(getattr(self.model, 'encoder', self.model), 'traits', None)
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            if getattr(self.trainable_module, 'is_feature_eval_model', False):
                features = self.trainable_module(input)
