...
from timm.models import create_model as create_timm_model, group_parameters
from timm.utils import unwrap_model
from timme.models import create_model as create_timme_model, is_registered
...
def _create_teacher_model(model_name: str, **kwargs) -> nn.Module:
    if is_registered(model_name):
        return create_timme_model(model_name, **kwargs)
    return create_timm_model(model_name=model_name, **kwargs)
...
class DistillationTeacher(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(model_name_or_module, str):
            ...
            model = _create_teacher_model(
                model_name_or_module,
                num_classes=num_classes,
                in_chans=in_chans,
                device=device,
                dtype=dtype,
...
class LogitDistillationTask(TrainingTask):
    ...
    def compile(
        ...
        self._maybe_disable_ddp_dynamo_optimizer(self.trainable_module)
    ...
    def get_eval_task(self, use_ema: bool = True):
        """Return a classification eval task for the student model."""
        from .eval_task import ClassificationEvalTask

        module = self.get_eval_model(ema=use_ema and self.has_ema())
        return ClassificationEvalTask(module)
...
class FeatureDistillationTask(TrainingTask):
    ...
    def compile(
        ...
        eval_model = torch.compile(raw_trainable.student, backend=backend, mode=mode, **compile_kwargs)
        raw_trainable.student = eval_model
