...
class TrainingTask(nn.Module):
    ...
    def prepare_distributed(
        ...
        if trainable_module is not None and trainable_module is not self:
            from torch.nn.parallel import DistributedDataParallel as DDP

            self.trainable_module = DDP(trainable_module, device_ids=device_ids, **ddp_kwargs)
        return self

    def _maybe_disable_ddp_dynamo_optimizer(self, module: Optional[nn.Module] = None) -> bool:
        """Disable Dynamo's DDP optimizer for DDP + activation checkpointing.

        PyTorch 2.9 and 2.11 can fail in this combination. Keep this close to
        compile() so programmatic task use gets the same protection as apps.
        """
        module = module if module is not None else self.get_trainable_module()
        try:
            from torch.nn.parallel import DistributedDataParallel as DDP
        except ImportError:
            return False
        if not isinstance(module, DDP):
            return False

        raw_module = unwrap_model(module)
        uses_grad_checkpointing = any(
            bool(getattr(m, 'grad_checkpointing', False))
            for m in raw_module.modules()
        )
        if not uses_grad_checkpointing:
            return False
        ...
        try:
            ...
            return False
        if not hasattr(dynamo.config, 'optimize_ddp') or not dynamo.config.optimize_ddp:
            return False

        dynamo.config.optimize_ddp = False
        if self.verbose:
            _logger.info(
                'Disabled Dynamo DDP optimizer for torch.compile + DDP + grad checkpointing.'
            )
        return True
    ...
    def compile(
        ...
        """Compile hot task components in their current wrapper state.
        ...
        """
        trainable_module = self.get_trainable_module()
        if trainable_module is None or trainable_module is self:
            return None
        self._maybe_disable_ddp_dynamo_optimizer(trainable_module)
    ...
    def get_checkpoint_state(
        ...
        state = {model_key: clean_state_dict(get_state_dict(eval_model, unwrap_fn))}
        if task_state:
    ...
    def load_checkpoint_state(
        ...
        unwrap_model(eval_model).load_state_dict(clean_state_dict(state_dict), strict=strict)
