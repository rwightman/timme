    ...
    if cfg.naflex.naflex_loader:
        ...
        if mixup_active:
            if cfg.loader.prefetcher:
                assert not num_aug_splits, 'Collate conflict: cannot use prefetcher mixup with aug splits'
...
def create_multiview_eval_loader(
    ...
    if is_primary(device_env):
        _logger.info(
            f'Created multi-view eval loader: {num_views} views, batch_size={batch_size}, input_size={input_size}'
        )
