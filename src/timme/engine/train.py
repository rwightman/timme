...
def train_one_epoch(
    ...
    trainable_module = task.get_trainable_module() if hasattr(task, 'get_trainable_module') else task
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        def _backward(_loss):
            if loss_scaler is not None:
                loss_scaler(
                    ...
                    parameters=task.get_clip_parameters(exclude_head='agc' in cfg.optimizer.clip_mode),
                ...
                if need_update:
                    if cfg.optimizer.clip_grad is not None:
                        dispatch_clip_grad(
                            task.get_clip_parameters(exclude_head='agc' in cfg.optimizer.clip_mode),
        ...
        if saver is not None and cfg.misc.recovery_interval and ((update_idx + 1) % cfg.misc.recovery_interval == 0):
