...
def setup_device(cfg: DeviceConfig, seed: int = 42) -> DeviceEnv:
    ...
    model_dtype = None
    if cfg.model_dtype:
        assert cfg.model_dtype in ('float32', 'float16', 'bfloat16'), f"Invalid model_dtype: {cfg.model_dtype}"
    ...
    if cfg.amp:
        ...
        assert cfg.amp_dtype in ('float16', 'bfloat16'), f"Invalid amp_dtype: {cfg.amp_dtype}"
