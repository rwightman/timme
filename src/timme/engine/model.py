...
from timme import create_encoder, create_model
...
def create_train_model(
    ...
    if target not in ('classifier', 'encoder'):
        raise ValueError(f"Unknown model target '{target}'. Expected 'classifier' or 'encoder'.")

    # Build factory kwargs for pretrained loading
    factory_kwargs = {}
    if cfg.pretrained_path:
        factory_kwargs['pretrained_cfg_overlay'] = dict(file=cfg.pretrained_path)
        if target == 'classifier':
            factory_kwargs['pretrained_cfg_overlay']['num_classes'] = -1  # force head adaptation
    ...
    # Head initialization
    if target == 'encoder' and (cfg.head_init_scale is not None or cfg.head_init_bias is not None):
        if is_primary(device_env):
            _logger.warning('Ignoring head initialization for encoder-only model target.')
    elif cfg.head_init_scale is not None:
        with torch.no_grad():
            model.get_classifier().weight.mul_(cfg.head_init_scale)
            model.get_classifier().bias.mul_(cfg.head_init_scale)

    if target == 'classifier' and cfg.head_init_bias is not None:
...
def get_naflex_patch_size(model: nn.Module) -> Optional[int]:
    ...
    if hasattr(model, 'get_patch_size'):
        return model.get_patch_size()
    if hasattr(model, 'encoder') and hasattr(model.encoder, 'patch_embed'):
        return getattr(model.encoder.patch_embed, 'patch_size', None)
